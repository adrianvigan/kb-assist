"""
Engineer Revision Portal
Engineers access this via revision link in email
"""
...
if not token:
    ...
    st.markdown("---")
    st.markdown("**Need help%s** Contact your KB manager for assistance.")
    st.stop()
...
if status == 'superseded':
    ...
    <div style="background-color: #d4edda; padding: 20px; border-left: 4px solid #28a745; border-radius: 5px; margin-top: 20px;">
        ...
        <h4 style="color: #155724;">What's Next%s</h4>
...
if status not in ['pending follow-up', 'rejected']:
    ...
    <div style="background-color: #d4edda; padding: 20px; border-left: 4px solid #28a745; border-radius: 5px; margin-top: 20px;">
        ...
        <hr style="border-color: #c3e6cb;">
        <h4 style="color: #155724;">What happened%s</h4>
        <ul style="color: #155724;">
...
st.caption("Need help improving your submission%s Use the AI tool to generate better content.")

with st.expander("🤖 Generate Improved Content", expanded=False):
    st.markdown("### What would you like to improve%s")
    ...
    if not revision_notes or not revision_notes.strip():
        ...
        try:
            ...
            if is_new_kb_request:
                # Handle new_kb_requests revision
